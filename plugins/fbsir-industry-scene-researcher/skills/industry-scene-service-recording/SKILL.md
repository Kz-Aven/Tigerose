...
来自 AI Agent 本地会话、任务账本、artifact-index、项目文件和可视化展示。可记录：
...
当用户想知道“还能如何用 AI Agent 更多能力支持当前行动”时，应进入宿主能力全景分层，而不是直接丢一串工具名。
...
当目标是为行业场景研究员提供“可复用的高绩效 AI Agent 使用模式”时，优先从创意库取材，而不是临时拼接工具名。
